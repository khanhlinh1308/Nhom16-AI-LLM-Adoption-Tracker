...
* **Total AI Events** — tổng số sự kiện liên quan đến AI đã bắt được.
* **Latest Batch** — số lượng sự kiện trong mẻ dữ liệu xử lý gần nhất.
* **Top Technology** — công nghệ AI được nhắc nhiều nhất trong khung giờ hiện tại.
* **Last Update** — thời gian hệ thống cập nhật dữ liệu lần cuối.
* **AI Activity over Time (Line chart)** — biểu đồ thể hiện biến động hoạt động theo thời gian.
* **Top AI Technologies (Bar chart)** — biểu đồ xếp hạng tần suất xuất hiện của các từ khóa công nghệ.
* **Top AI Repositories (bảng)** — danh sách các repository có hoạt động liên quan đến AI sôi nổi nhất (theo Event Count), click vào tên để mở GitHub gốc.

Các chỉ số này được Spark tính toán theo cửa sổ thời gian (mặc định 1 phút) và lưu trong Redis với các key tương ứng: `top_ai_keywords`, `top_ai_repos`, `total_ai_events_detected`, `ai_activity_timeline`.
...
| Thành viên | Vai trò |
|---|---|
| Linh | Project Lead, Tài liệu đặc tả & thiết kế, UI/UX Dashboard, Báo cáo & Slide, Video demo |
